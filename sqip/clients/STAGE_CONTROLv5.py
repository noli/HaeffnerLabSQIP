...
class XYpos (QtGui.QWidget):
    def __init__(self, reactor, parent=None):
        ...
        self.button['posy1'] = QtGui.QPushButton('v')    #backwards b/c y stage 0 is highest point
    ...
    def connect(self):
        ...
        self.stageserver = yield self.cxn.sqip_expcontrol_stage_server
        ...
        if self.buttonPushed:
            if self.dirn == 'pos':
                ...
                if newVal < 0: 
                    ...
                    self.control[self.axis].setValue(float(self.control[self.axis].value() - self.delta[self.axis].value()))
            self.controlInputUpdated = True
            self.position[self.axis] = round(self.control[self.axis].value(), 6)
            self.updatedControl = self.axis
        self.buttonPushed = False
